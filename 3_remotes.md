...
24. ```git push http://github.com/johansundstrom/gitlab.git``` alternativt ```git push origin master```
...
28. ```git fetch <url>``` Hämta förändringar från origin
29. ```git remote``` Lista anslutningar (```origin``` är standard kortnamn på anslutning)
30. ```git remote -v``` Lista anslutningar verbose
